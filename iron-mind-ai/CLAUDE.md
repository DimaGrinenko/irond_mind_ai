...
- AI chat uses `AiProvider` (`backend/src/ai/ai-provider.ts`): **DeepSeek** (default if `DEEPSEEK_API_KEY`), **Groq**, generic OpenAI-compatible, or **Anthropic**. Rule-based fallback when no key or `AI_PROVIDER=off`. Setup: `AI_SETUP_RU.txt`.
...
- Gamification/economy (leaves, shop, friends, duels, challenges) syncs with backend wallet/shop; friends/duels sharing still partial mock.
