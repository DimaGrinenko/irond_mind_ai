...
export class AchievementsService {
  ...
  async getTree(userId: string) {
    ...
    return achievements.map((a) => {
      ...
      return {
        ...
        status: ua?.status ?? AchievementStatus.LOCKED,
  ...
  // Called from WorkoutsService when a workout is finished.
  async recordWorkoutCompletion(userId: string, workoutTitle?: string) {
    ...
    await this.prisma.activityEvent.create({
      data: {
        userId,
        kind: ActivityEventKind.WORKOUT_COMPLETED,
        title: workoutTitle ?? 'Тренировка завершена',
        xp: xpGain,
      },
    });

    // Считаем глобальные показатели для ачивок
    ...
    const totalVolumeKg = allTimeSets.reduce(
  ...
  private async checkAchievements(
    ...
    // Обновляем ВСЕ ачивки текущей категории (включая LOCKED) — чтобы был прогресс
    const vol = Math.round(ctx.totalVolumeKg);
    const updates: Array<{ id: string; currentValue: number; targetValue: number; reward: { xp: number; leaves: number } }> = [
    ...
    for (const u of updates) {
      ...
      if (existing && existing.status === AchievementStatus.UNLOCKED) continue;

      const isUnlocked = u.currentValue >= u.targetValue;
      const isInProgress = u.currentValue > 0;
      const data: Prisma.UserAchievementUpsertArgs['create'] = {
        userId,
        achievementId: u.id,
      ...
      if (isUnlocked) {
        ...
        await this.prisma.activityEvent.create({
          data: {
            userId,
            kind: ActivityEventKind.ACHIEVEMENT_UNLOCKED,
  ...
  private isSameDay(a: Date | null | undefined, b: Date) {
    if (!a) return false;
    return a.getFullYear() === b.getFullYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate();
  }

  private isYesterday(a: Date, b: Date) {
