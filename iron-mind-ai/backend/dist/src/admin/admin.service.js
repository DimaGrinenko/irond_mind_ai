...
let AdminService = class AdminService {
    ...
    async assignCoach(clientId, coachId, notes) {
        ...
        if (!client || !coach)
            throw new common_1.NotFoundException();
        if (coach.role !== client_1.UserRole.COACH && coach.role !== client_1.UserRole.ADMIN) {
            throw new common_1.NotFoundException('Пользователь не является тренером');
        }
