    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                ...
                >
                  <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 18 }}>
                    {(userName || 'А').slice(0, 1).toUpperCase()}
                  </Text>
                </LinearGradient>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 16 }}>{userName || t('profile.athlete')}</Text>
                ...
                >
                  <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 12 }}>{t('profile.premium')}</Text>
                </Pressable>
              </View>

              <View style={{ marginTop: 14, flexDirection: 'row', justifyContent: 'space-between', flexWrap: 'wrap', rowGap: 14 }}>
                {([
                  { label: t('profile.achievements'), icon: 'trophy-outline' as const, go: () => navigation.navigate('Achievements') },
                  { label: t('home.workouts'), icon: 'barbell-outline' as const, go: () => navigation.navigate('WorkoutsHistory') },
                  { label: t('profile.supplements'), icon: 'flask-outline' as const, go: () => navigation.navigate('Supplements') },
                  { label: t('profile.calendar'), icon: 'calendar-outline' as const, go: () => navigation.navigate('Calendar') },
                  { label: t('profile.qa.challenges'), icon: 'flag-outline' as const, go: () => navigation.navigate('Challenges') },
                  { label: t('profile.qa.shop'), icon: 'leaf-outline' as const, go: () => navigation.navigate('LeafShop') },
                  { label: t('profile.qa.friends'), icon: 'people-outline' as const, go: () => navigation.navigate('Friends') },
                  { label: t('profile.qa.wellbeing'), icon: 'pulse-outline' as const, go: () => navigation.navigate('WellbeingDiary') },
                  userGender === 'female'
                    ? { label: t('profile.qa.cycle'), icon: 'moon-outline' as const, go: () => navigation.navigate('CycleTracker') }
                    : null,
                  { label: t('profile.qa.aiPlan'), icon: 'sparkles-outline' as const, go: () => navigation.navigate('AiProgramGen') },
                  { label: t('profile.qa.coach'), icon: 'medal-outline' as const, go: () => navigation.navigate('LiveCoach') },
                  { label: t('profile.qa.health'), icon: 'heart-outline' as const, go: () => navigation.navigate('HealthSync') },
                ].filter(Boolean) as Array<{ label: string; icon: any; go: () => void }>).map((x) => (
                  <Pressable key={x.label} onPress={x.go} style={{ alignItems: 'center', width: 74 }}>
                    <View
                      style={{
                        width: 54,
                        height: 54,
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
          <Text style={{ color: colors.textSecondary, fontFamily: fontFamilies.body600, fontSize: 12 }}>{t('profile.settings')}</Text>
        </View>

        <View style={{ paddingHorizontal: 16, marginTop: 10, gap: 12 }}>
          {[
            { label: t('profile.goal'), v: goalLabel(userGoalKey) || t('profile.goalNotSet'), icon: 'flag-outline' as const, go: undefined },
            {
            ...
            },
            {
              ...
              v: currentProgramId ? (programTitle ?? t('profile.programActive')) : t('profile.programNone'),
              icon: 'barbell-outline' as const,
              go: () =>
                currentProgramId
                  ? navigation.navigate('ProgramDetail', { programId: currentProgramId })
                  : navigation.navigate('RootTabs', { screen: 'Programs' }),
            },
            {
              label: t('profile.sync'),
              v: authOnline ? `${authEmail ?? t('profile.online')}` : t('profile.offline'),
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 13 }}>{x.label}</Text>
          ...
          {isCoach ? (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.cyan, fontFamily: fontFamilies.body700, fontSize: 13 }}>{t('profile.coachPanel')}</Text>
                <Text style={{ marginTop: 3, color: colors.textSecondary, fontSize: 12 }}>{t('profile.coachPanelSub')}</Text>
              </View>
              <Ionicons name="chevron-forward" size={22} color={colors.textMuted} />
            </Pressable>
          ) : null}
          ...
          {isAdmin ? (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.pink, fontFamily: fontFamilies.body700, fontSize: 13 }}>{t('profile.adminPanel')}</Text>
                <Text style={{ marginTop: 3, color: colors.textSecondary, fontSize: 12 }}>{t('profile.adminPanelSub')}</Text>
              </View>
              <Ionicons name="chevron-forward" size={22} color={colors.textMuted} />
            </Pressable>
          ) : null}
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 10, gap: 12 }}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ color: colors.red, fontFamily: fontFamilies.body700, fontSize: 13 }}>{t('profile.logout')}</Text>
...
function LanguageSwitcher() {
  ...
  return (
    <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
      <Text style={{ color: colors.textSecondary, fontFamily: fontFamilies.body600, fontSize: 12, marginBottom: 10 }}>
        {t('profile.language')}
      </Text>
