...
function getMuscleUI() {
  return [
    {
      key: 'chest' as const,
      name: t('analytics.muscleChest'),
      color: colors.pink,
    },
    {
      key: 'back' as const,
      name: t('analytics.muscleBack'),
      color: colors.blue,
    },
    {
      key: 'legs' as const,
      name: t('analytics.muscleLegs'),
      color: colors.green,
    },
    {
    ...
    {
      key: 'arms' as const,
      name: t('analytics.muscleArms'),
      color: colors.red,
    },
    {
      key: 'core' as const,
      name: t('analytics.muscleCore'),
      color: colors.amber,
    },
  ];
}
...
export function AnalyticsScreen() {
  ...
  const isEmpty =
    (stats?.workoutsCount ?? 0) === 0 && (stats?.nutritionCalories ?? 0) === 0;
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      >
        ...
        >
          ...
          >
            <Text
              style={{ color: colors.cyan, fontFamily: fontFamilies.body600 }}
            >
        ...
        {loading && !stats ? (
          ...
          <>
              ...
              <Card variant="secondary" style={{ paddingVertical: 12 }}>
                ...
                >
                  <StatCell
                    ...
                    value={(stats?.workoutsCount ?? 0).toString()}
                  />
                  <StatCell
                  ...
                  <StatCell
                    ...
                    value={`${(stats?.volumeKg ?? 0).toLocaleString('ru')} ${t('common.kg')}`}
                    small
                  />
                </View>
                <View
                ...
                <View style={{ paddingVertical: 12, paddingHorizontal: 12 }}>
                  ...
                  <LineChart
                    ...
                    series={[
                      ...
                      {
                        points: wNorm.length ? wNorm : new Array(days).fill(0),
                        gradientId: 'gradBlue',
                        stroke: blue,
                      },
                    ]}
                  ...
                  <View style={{ width: 120 }}>
                    <BodyFigure
                      ...
                      highlights={{
                        chest: alphaColor(
                          colors.pink,
                          0.15 + 0.7 * (stats?.muscleLoad.chest ?? 0),
                        ),
                        back: alphaColor(
                          colors.blue,
                          0.15 + 0.7 * (stats?.muscleLoad.back ?? 0),
                        ),
                        legs: alphaColor(
                          colors.green,
                        ...
                        arms: alphaColor(
                          colors.red,
                          0.15 + 0.7 * (stats?.muscleLoad.arms ?? 0),
                        ),
                        core: alphaColor(
                          colors.amber,
                          0.15 + 0.7 * (stats?.muscleLoad.core ?? 0),
                        ),
                      }}
                    />
                  </View>
            ...
            {stats &&
              (() => {
                const entries: Array<{
                  key: keyof typeof stats.muscleLoad;
                  name: string;
                  v: number;
                }> = MUSCLE_UI.map((x) => ({
                  key: x.key,
                  name: x.name,
                  v: stats.muscleLoad[x.key] ?? 0,
                }));
                    ...
                    >
                      ...
                      >
                        {t('analytics.laggingHint', {
                          groups: lowest
                            .map((e) => `${e.name} (${Math.round(e.v * 100)}%)`)
                            .join(', '),
                        })}
                      </Text>
                    </Card>
            ...
            {stats && stats.muscleLoad
              ? (() => {
                  ...
                  const loaded = muscleLoadValues.filter(
                    (v) => v >= 0.3,
                  ).length;
            ...
            <View style={{ paddingHorizontal: 16, marginTop: 12, gap: 8 }}>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                <ToolBtn
                  icon="images-outline"
                  label={t('analytics.tools.before')}
                  tint={colors.pink}
                  onPress={() => nav.navigate('PhotoCompare')}
                />
                <ToolBtn
                  icon="download-outline"
                  label={t('analytics.tools.csv')}
                  tint={colors.cyan}
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <ToolBtn
                  icon="flag-outline"
                  label={t('analytics.tools.challenges')}
                  tint={colors.amber}
                  onPress={() => nav.navigate('Challenges')}
                />
                <ToolBtn
                  icon="leaf-outline"
                  label={t('analytics.tools.shop')}
                  tint={colors.green}
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <ToolBtn
                  ...
                  label={t('analytics.tools.form')}
                  tint={colors.pink}
                  onPress={() => nav.navigate('FormAnalysis')}
                />
              </View>
            </View>
          </>
