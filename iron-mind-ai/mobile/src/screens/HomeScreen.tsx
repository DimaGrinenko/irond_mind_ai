...
import { api, type ProgramFull, type UserDashboard, type ScheduledWorkout } from '../api/client';
import { t, useLang, goalLabel, programLabel, dayTitle } from '../i18n';

const VOLUME_PER_LEVEL = 5000; // 5 тонн = +1 уровень

function levelFromVolume(kg: number): { level: number; progress: number; toNext: number } {
  const level = Math.floor(kg / VOLUME_PER_LEVEL) + 1;
  const inLevel = kg % VOLUME_PER_LEVEL;
  return { level, progress: inLevel / VOLUME_PER_LEVEL, toNext: VOLUME_PER_LEVEL - inLevel };
}
      ...
      >
        <View style={{ paddingTop: insets.top + 8, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <NeonText variant="eyebrow" glow="cyan" glowRadius={10}>AI TRAINER</NeonText>
          </View>
          <View style={{ flex: 1 }} />
          <Pressable
            ...
            style={{
              ...
              borderWidth: 1, borderColor: 'rgba(157,107,255,0.55)',
              alignItems: 'center', justifyContent: 'center',
              backgroundColor: 'rgba(13,16,32,0.7)',
              shadowColor: colors.purple, shadowOpacity: 0.6, shadowRadius: 14, shadowOffset: { width: 0, height: 0 }, elevation: 8,
            }}
          >
        ...
        <AppearUp delayMs={0}>
          <View style={{ paddingHorizontal: 16, marginTop: 18 }}>
            ...
            <Text style={[
              ...
              {userName || t('profile.athlete')} <Text style={{ fontSize: 22 }}>👋</Text>
            </Text>

            {userGoalKey ? (
              <>
                <NeonText variant="eyebrow" glow="none" style={{ marginTop: 14, color: colors.textMuted }}>{t('home.goal')}</NeonText>
                <Text style={[
                  { marginTop: 4, color: colors.text, fontFamily: fontFamilies.body600 },
                  neonTextShadow('rgba(157,107,255,0.35)', 8),
                ]}>{goalLabel(userGoalKey)}</Text>
              </>
            ) : null}
          </View>
        </AppearUp>
        ...
        <AppearUp delayMs={80}>
          <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
            {program ? (
              <Pressable onPress={() => nav.navigate('ProgramDetail', { programId: program.id })}>
                  ...
                  <View style={{ padding: 16, flexDirection: 'row', gap: 14, alignItems: 'center' }}>
                    <View
                      style={{
                        ...
                        borderWidth: 1, borderColor: 'rgba(157,107,255,0.6)',
                        alignItems: 'center', justifyContent: 'center',
                        shadowColor: colors.purple, shadowOpacity: 0.7, shadowRadius: 16, shadowOffset: { width: 0, height: 0 }, elevation: 8,
                      }}
                    >
                      <Ionicons name={(program.iconName || 'barbell') as any} size={24} color={colors.purpleLight} />
                    </View>
              ...
              <NeonCard tint="aurora" intensity={0.95}>
                <View style={{ padding: 16 }}>
                  ...
                  <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 16, marginTop: 6 }}>
                  ...
                  <View style={{ marginTop: 14, flexDirection: 'row', gap: 10 }}>
                    ...
                    >
                      <Ionicons name="construct-outline" size={16} color={colors.purpleLight} />
                      <Text style={{ color: colors.purpleLight, fontFamily: fontFamilies.body700, fontSize: 12 }}>
                        {t('home.composePlan')}
                      </Text>
                    </Pressable>
                    <Pressable
                      onPress={() => nav.navigate('RootTabs', { screen: 'Programs' })}
                      style={{
                        flex: 1,
                        paddingVertical: 12,
                        borderRadius: 14,
              ...
              <View style={{ padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'space-between' }}>
                  <View style={{ alignItems: 'flex-start' }}>
                    <NeonText variant="eyebrow" glow="none" style={{ color: colors.textMuted }}>{t('home.workouts')}</NeonText>
                    <AnimatedCounter
                      to={totalWorkouts}
                      style={[
                  ...
                  <View style={{ alignItems: 'flex-start' }}>
                    <NeonText variant="eyebrow" glow="none" style={{ color: colors.textMuted }}>{t('home.level')}</NeonText>
                    <AnimatedCounter
                      to={lvl.level}
                      duration={900}
                      style={[
                    ...
                    <Text style={{ color: colors.textMuted, fontFamily: fontFamilies.body, fontSize: 10, marginTop: 2 }}>
                      {t('home.volumeKg', { v: totalVolume.toLocaleString('ru') })}
                    </Text>
                  </View>
                </View>
                <View style={{ alignItems: 'center' }}>
                  <StreakDumbbell days={streakDays} size={72} />
                  <NeonText variant="eyebrow" glow="amber" glowRadius={8} style={{ marginTop: 10, fontSize: 10 }}>
              ...
              <NeonCard tint="aurora" intensity={0.95}>
                <View style={{ padding: 16, flexDirection: 'row', gap: 12 }}>
                  <View style={{ flex: 1 }}>
                    {todayDone ? (
                      <>
                        <NeonText variant="eyebrow" glow="cyan" glowRadius={10}>{t('home.today').toUpperCase()}</NeonText>
                        <Text style={[
                        ...
                        <View style={{ marginTop: 14 }}>
                          <GradientButton
                            ...
                            rightIcon={<Ionicons name="time" size={18} color="#fff" />}
                      ...
                      <>
                        <NeonText variant="eyebrow" glow="pink" glowRadius={10}>{t('home.today').toUpperCase()}</NeonText>
                        <Text style={[
                          { marginTop: 10, color: colors.text, fontFamily: fontFamilies.body700, fontSize: 18 },
                        ...
                        <Text style={{ marginTop: 4, color: colors.textSecondary, fontFamily: fontFamilies.body }}>
                          {todayDay.exercises.length} {t('common.exShort')} · {programLabel(program.id, program.title)}
                        </Text>
                      ...
                      <>
                        <NeonText variant="eyebrow" glow="pink" glowRadius={10}>{t('home.composePlan').toUpperCase()}</NeonText>
                        <Text style={[
                          { marginTop: 10, color: colors.text, fontFamily: fontFamilies.body700, fontSize: 18 },
                          neonTextShadow('rgba(255,77,210,0.65)', 14),
                        ]}>
                        ...
                        <View style={{ marginTop: 14 }}>
                          <GradientButton
                            ...
                            onPress={() => nav.navigate('ProgramDetail', { programId: program.id })}
                          />
                        </View>
                      </>
                    )}
        ...
        <AppearUp delayMs={230}>
          <View style={{ paddingHorizontal: 16, marginTop: 12 }}>
            <Pressable onPress={() => nav.navigate('RootTabs', { screen: 'AiTrainer' })}>
              <NeonCard tint="cyan" intensity={0.6}>
                <View style={{ padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  ...
                  <View style={{ flex: 1 }}>
                    <NeonText variant="eyebrow" glow="cyan" glowRadius={8} style={{ fontSize: 10 }}>
                      {t('home.aiTip').toUpperCase()}
                    </NeonText>
                    <Text style={{ marginTop: 4, color: colors.text, fontFamily: fontFamilies.body600, fontSize: 13 }}>
                      {todayDone
                        ? t('home.todayMissing')
                        : todayDay
                          ? t('home.todayActive', { title: dayTitle(todayDay.title) })
                          : t('home.pickProgFirst')}
                    </Text>
                    <Text style={{ marginTop: 6, color: colors.cyan, fontFamily: fontFamilies.body700, fontSize: 11 }}>
                      {t('home.aiTipMore')} →
                    </Text>
                  </View>
                </View>
        ...
        <AppearUp delayMs={260}>
          <View style={{ paddingHorizontal: 16, marginTop: 18 }}>
            <NeonText variant="eyebrow" glow="purple" glowRadius={10}>{t('home.quickActions')}</NeonText>
            <View style={{ marginTop: 12, flexDirection: 'row', justifyContent: 'space-between' }}>
              {(
                [
                  { label: t('home.measurements'), icon: 'body-outline', tint: colors.cyan, go: () => nav.navigate('BodyMeasurements') },
                  { label: t('home.nutrition'), icon: 'restaurant-outline', tint: colors.green, go: () => nav.navigate('Nutrition') },
                  { label: t('home.photos'), icon: 'camera-outline', tint: colors.amber, go: () => nav.navigate('ProgressPhotos') },
                  { label: t('home.analytics'), icon: 'analytics-outline', tint: colors.pink, go: () => nav.navigate('RootTabs', { screen: 'Analytics' }) },
                ] as const
              ).map((x) => (
                <TapScale
                  key={x.label}
                ...
                >
                  <View
                    style={{
                      ...
                      borderWidth: 1, borderColor: x.tint + 'AA',
                      alignItems: 'center', justifyContent: 'center',
                      shadowColor: x.tint, shadowOpacity: 0.7, shadowRadius: 14, shadowOffset: { width: 0, height: 0 }, elevation: 8,
                    }}
                  >
                    <Ionicons name={x.icon as any} size={22} color={x.tint} />
                  </View>
                  <Text style={{ marginTop: 8, color: colors.textSecondary, fontFamily: fontFamilies.body500, fontSize: 11, textAlign: 'center' }}>
                    {x.label}
                  </Text>
                </TapScale>
              ))}
