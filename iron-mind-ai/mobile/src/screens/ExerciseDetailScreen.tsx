...
export function ExerciseDetailScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <ScreenHeader
        ...
        right={
          hasVideo ? (
            ...
            >
              <Ionicons name="open-outline" size={14} color={colors.purpleLight} />
            </Pressable>
          ) : null
        }
      />
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 130 }} showsVerticalScrollIndicator={false}>
            ...
            <LinearGradient colors={['rgba(79,31,184,0.22)', 'rgba(0,0,0,0)']} style={{ padding: 16 }}>
              ...
              <View
                style={{
                  ...
                  overflow: 'hidden',
                  borderWidth: 1,
                  borderColor: hasVideo ? 'rgba(123,63,228,0.55)' : 'rgba(42,42,62,0.9)',
                  aspectRatio: 16 / 9,
              ...
              >
                {videoOpen && ytId && Platform.OS !== 'web' ? (
                  ...
                  >
                    ...
                    <Text style={{ marginTop: 8, color: colors.textSecondary, fontFamily: fontFamilies.body, fontSize: 12 }}>
                      Tap to open
                    </Text>
                  </Pressable>
                ) : (
                  ...
                  >
                    <View
                      style={{
                        ...
                        height: 64,
                        borderRadius: 32,
                        backgroundColor: hasVideo ? 'rgba(123,63,228,0.45)' : 'rgba(0,0,0,0.45)',
                        borderWidth: 1,
                    ...
                    <Text style={{ marginTop: 10, color: colors.textSecondary, fontFamily: fontFamilies.body600, fontSize: 12 }}>
                      {hasVideo ? 'Тап чтобы посмотреть' : 'Видео скоро'}
                    </Text>
                  </Pressable>
                )}
              </View>
                ...
                <View style={{ width: 130, alignItems: 'center' }}>
                  <BodyFigure
                    ...
                    highlights={{
                      ...
                      legs: ex.primary === 'Ноги' || ex.primary === 'Ягодицы' ? colors.green : 'rgba(255,255,255,0.03)',
                ...
                <View style={{ flex: 1, paddingLeft: 12 }}>
                  ...
                  <View style={{ marginTop: 8, gap: 6 }}>
                    {[ex.primary, ...ex.secondary].slice(0, 4).map((m, i) => (
                      <View key={`${m}-${i}`} style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <View
                          style={{
                            width: 6,
                            height: 6,
                            borderRadius: 3,
                            backgroundColor: i === 0 ? colors.pink : colors.textMuted,
                          }}
                        />
                        <Text style={{ color: i === 0 ? colors.text : colors.textSecondary, fontFamily: fontFamilies.body600, fontSize: 12 }}>
                          {m}{i === 0 ? ' (основная)' : ''}
                        </Text>
                      </View>
                    ))}
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
          <Card variant="secondary" style={{ padding: 14 }}>
            ...
            <View style={{ gap: 10 }}>
              {ex.steps.map((s, idx) => (
                <View key={`${s}-${idx}`} style={{ flexDirection: 'row', gap: 10 }}>
                  <View
                    style={{
                      width: 24,
                      height: 24,
                  ...
                  >
                    <Text style={{ color: colors.purpleLight, fontFamily: fontFamilies.body700, fontSize: 11 }}>{idx + 1}</Text>
                  </View>
                  <Text style={{ flex: 1, color: colors.textSecondary, fontFamily: fontFamilies.body, fontSize: 13 }}>
                    {s}
                  </Text>
                </View>
              ))}
        ...
        {ex.tips.length > 0 ? (
          <View style={{ paddingHorizontal: 16, marginTop: 14 }}>
            <Card variant="secondary" style={{ padding: 14 }}>
              ...
              <View style={{ gap: 6 }}>
                {ex.tips.map((tip, idx) => (
                  <Text key={`${tip}-${idx}`} style={{ color: colors.textSecondary, fontFamily: fontFamilies.body, fontSize: 13 }}>
                    • {tip}
                  </Text>
                ))}
              </View>
      ...
      >
        <GradientButton
          title={activeWorkout ? 'Вернуться к тренировке' : 'Начать тренировку'}
          onPress={() => {
...
function Tag({ children }: { children: React.ReactNode }) {
  return (
    ...
    >
      <Text style={{ color: colors.textSecondary, fontFamily: fontFamilies.body600, fontSize: 11 }}>
        {children}
      </Text>
    </View>
  );
...
function YoutubeEmbed({ videoId }: { videoId: string }) {
  ...
  React.useEffect(() => {
    import('react-native-webview').then((m) => setWebViewComp(() => m.WebView)).catch(() => setWebViewComp(null));
  }, []);
  if (!WebViewComp) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
        <Text style={{ color: colors.textMuted, fontFamily: fontFamilies.body, fontSize: 12 }}>Loading…</Text>
      </View>
    );
  }
