...
const EMPTY = {
  neck: '', shoulders: '', chest: '', waist: '', forearm: '',
  hips: '', thigh: '', biceps: '', calf: '', weight: '',
};
...
export function BodyMeasurementsScreen() {
  ...
  const [bjuSuggestion, setBjuSuggestion] = useState<ReturnType<typeof computeBju> | null>(null);
  ...
  const onSubmit = async () => {
    try {
      ...
      if (newWeight && newWeight > 0) {
        ...
        if (suggestion && Math.abs(suggestion.calories - currentCalories) >= 50) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <View style={{ paddingTop: insets.top + 8, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center' }}>
        ...
        <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 18 }}>{t('meas.title')}</Text>
        <View style={{ flex: 1 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        {bjuSuggestion ? (
          <View style={{ paddingHorizontal: 16, marginTop: 12 }}>
            ...
            >
              ...
              <Text style={{ color: colors.text, fontFamily: fontFamilies.body, fontSize: 12, marginBottom: 8 }}>
                {t('meas.bjuLine', {
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                ...
                >
                  <Text style={{ color: colors.cyan, fontFamily: fontFamilies.body700, fontSize: 12 }}>{t('meas.bjuApply')}</Text>
                </Pressable>
                <Pressable
            ...
            <LinearGradient colors={['rgba(79,31,184,0.18)', 'rgba(0,0,0,0)']} style={{ padding: 14 }}>
              <View style={{ flexDirection: 'row', gap: 12 }}>
                <View style={{ width: 110 }}>
                  <FieldRow label={t('meas.neck')} value={form.neck} onChange={(v) => setForm((s) => ({ ...s, neck: v }))} compact />
                  <FieldRow label={t('meas.chest')} value={form.chest} onChange={(v) => setForm((s) => ({ ...s, chest: v }))} compact />
                  <FieldRow label={t('meas.waist')} value={form.waist} onChange={(v) => setForm((s) => ({ ...s, waist: v }))} compact />
                  <FieldRow label={t('meas.shoulders')} value={form.shoulders} onChange={(v) => setForm((s) => ({ ...s, shoulders: v }))} compact />
                  <FieldRow label={t('meas.forearm')} value={form.forearm} onChange={(v) => setForm((s) => ({ ...s, forearm: v }))} compact />
                </View>
                ...
                <View style={{ width: 110 }}>
                  <FieldRow label={t('meas.hips')} value={form.hips} onChange={(v) => setForm((s) => ({ ...s, hips: v }))} compact />
                  <FieldRow label={t('meas.thigh')} value={form.thigh} onChange={(v) => setForm((s) => ({ ...s, thigh: v }))} compact />
                  <FieldRow label={t('meas.biceps')} value={form.biceps} onChange={(v) => setForm((s) => ({ ...s, biceps: v }))} compact />
                  <FieldRow label={t('meas.calf')} value={form.calf} onChange={(v) => setForm((s) => ({ ...s, calf: v }))} compact />
                  <FieldRow label={t('meas.weight')} value={form.weight} onChange={(v) => setForm((s) => ({ ...s, weight: v }))} suffix={t('common.kg')} compact />
                </View>
              </View>
            </LinearGradient>
          </Card>
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 14, flexDirection: 'row', gap: 10 }}>
          <View style={{ flex: 1 }}>
            <GradientButton title={t('meas.add')} disabled={!canSave} onPress={onSubmit} />
          </View>
          {canSave ? (
            <Pressable
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 10, gap: 10 }}>
          {rows.length === 0 ? (
            ...
            rows.map((r) => <HistoryItem key={r.id} row={r} onDelete={() => onDelete(r.id)} />)
          )}
        </View>
      </ScrollView>
    </View>
...
function HistoryItem({ row, onDelete }: { row: MeasurementsRow; onDelete: () => void }) {
  const fields: Array<{ label: string; v: number | null | undefined; suffix?: string }> = [
    { label: t('meas.weight'), v: row.weight, suffix: t('common.kg') },
    { label: t('meas.chest'), v: row.chest, suffix: 'см' },
    { label: t('meas.waist'), v: row.waist, suffix: 'см' },
    { label: t('meas.biceps'), v: row.biceps, suffix: 'см' },
    { label: t('meas.thigh'), v: row.thigh, suffix: 'см' },
    { label: t('meas.shoulders'), v: row.shoulders, suffix: 'см' },
    { label: t('meas.hips'), v: row.hips, suffix: 'см' },
    { label: t('meas.neck'), v: row.neck, suffix: 'см' },
    { label: t('meas.calf'), v: row.calf, suffix: 'см' },
    { label: t('meas.forearm'), v: row.forearm, suffix: 'см' },
  ].filter((f) => f.v !== null && f.v !== undefined);

  return (
    <Card variant="secondary" style={{ padding: 14 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
        <Ionicons name="calendar-outline" size={14} color={colors.purpleLight} style={{ marginRight: 6 }} />
        <Text style={{ flex: 1, color: colors.text, fontFamily: fontFamilies.body700, fontSize: 13 }}>
          {formatDateRu(row.date)}
        </Text>
        <Pressable onPress={onDelete} hitSlop={10} style={{ paddingHorizontal: 6 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
          {fields.map((f) => (
            ...
            >
              ...
              <Text style={{ color: colors.text, fontFamily: fontFamilies.body700, fontSize: 13, marginTop: 2 }}>
                {f.v} {f.suffix ?? ''}
              </Text>
            </View>
          ))}
