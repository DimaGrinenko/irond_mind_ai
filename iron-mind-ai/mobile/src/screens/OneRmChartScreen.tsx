...
export function OneRmChartScreen() {
  ...
  const max1rm = useMemo(() => data.reduce((m, d) => (d.max1rm > m ? d.max1rm : m), 0), [data]);
  const min1rm = useMemo(() => data.reduce((m, d) => (m === 0 || d.max1rm < m ? d.max1rm : m), 0), [data]);
  const range = Math.max(1, max1rm - min1rm);
  const norm = data.map((d) => Math.max(4, ((d.max1rm - min1rm) / range) * 100));
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ paddingHorizontal: 16, marginTop: 12 }}>
          <Card variant="secondary" style={{ padding: 16 }}>
            <Text style={{ color: colors.textMuted, fontFamily: fontFamilies.body500, fontSize: 10, letterSpacing: 1, marginBottom: 10 }}>
              {t('oneRm.chartHeader')}
            </Text>

            {loading ? (
              <ActivityIndicator color={colors.purpleLight} style={{ paddingVertical: 30 }} />
            ) : data.length === 0 ? (
              <Text style={{ color: colors.textMuted, fontFamily: fontFamilies.body, fontSize: 12, textAlign: 'center', paddingVertical: 24 }}>
                {t('oneRm.empty')}
              </Text>
            ) : (
              <>
                <LineChart
                  ...
                  height={140}
                  series={[
                    { points: norm, gradientId: 'gradPurple', stroke: colors.purpleLight },
                  ]}
                />
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 }}>
                  <Text style={{ color: colors.textMuted, fontSize: 10, fontFamily: fontFamilies.body500 }}>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 14 }}>
                  <Stat label={t('oneRm.current')} value={`${last?.max1rm.toFixed(1) ?? '—'} ${t('common.kg')}`} tint={colors.purpleLight} />
                  <Stat label={t('oneRm.growth')} value={`${growth >= 0 ? '+' : ''}${growth.toFixed(1)} ${t('common.kg')}`} tint={growth >= 0 ? colors.green : colors.pink} />
                  <Stat label={t('oneRm.points')} value={String(data.length)} tint={colors.cyan} />
                </View>
              </>
            )}
          </Card>
...
function Stat({ label, value, tint }: { label: string; value: string; tint: string }) {
  return (
    ...
    >
      <Text style={{ color: colors.textMuted, fontFamily: fontFamilies.body500, fontSize: 9, letterSpacing: 0.5 }}>
        {label.toUpperCase()}
      </Text>
      <Text style={{ marginTop: 4, color: tint, fontFamily: fontFamilies.body700, fontSize: 14 }}>{value}</Text>
    </View>
  );
}
